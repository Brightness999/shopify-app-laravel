...
class ShopifyWebHooksController extends Controller
{
    ...
    if(!$res['id'] > 0){
        ...
        $id_customer = 0;
        try {
            foreach ($request->line_items as $line) {
                //Log::info(json_encode($line['product_id']));
            ...
            if (count($order_details_list) > 0) {
                ...
                //$order->total = $total_green + $order->shipping_price;
                ...
                try {
                    ...
                    if (count($col)) {//has at least an item?
                        ...
                        if ($met != null) {
                            ...
                            Log::info('Shipping price' . $order->shipping_price);
                            Log::info('method_code' . $order->shipping_method_code);
                            Log::info('carrier_code' . $order->shipping_carrier_code);
                            Log::info('shipping_title' . $order->shipping_title);
                            //Log::info('Carrier Type' . $met->carrier_type);
                            Log::info('LOG-CARTFROMMAGENTO');
