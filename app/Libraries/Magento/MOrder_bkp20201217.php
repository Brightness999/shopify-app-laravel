...
class MOrder_bkp20201217
{
    ...
    {
        $api = MagentoApi::getInstance();
        Log::info("CUSTOMPAULO-ORDERMODEL");
        Log::info($order_model);

        //get shop information
        $merchant = User::find($order_model->id_customer);
        $res = ShopifyAdminApi::getShopInformation($merchant);
        $ad1 = $res['address1'];
        Log::info("merchant id: ".$merchant);
        $shx = Order::where('id', $order_model->id)->first();
        ...
        $shipping = [
            'addressInformation' => [
                ...
                'shipping_carrier_code' => $shx->shipping_carrier_code,
                'shipping_method_code' => $shx->shipping_method_code,
            ]
        ];

        Log::info('Informacion del shipping');
        Log::info($shipping);

        $querymg = DB::connection('mysql_magento')->select('SELECT * FROM `mg_quote_id_mask` WHERE quote_id = ' . $order_model->magento_quote_id);
        $maskid = 0;
        Log::info('id mask con el quote id desde la app');
        Log::info($querymg);
        if (count($querymg)) {
            //dd($querymg);
            $maskid = $querymg[0]->masked_id;
        ...
        Log::info('paymentmethyodsRes desl post guest carts shipping information');
        Log::info($paymentMethodsRes);
        $paymentInformation = [
            'paymentMethod' => [
                'method' => 'checkmo'
            ]
    ...
    /*    
        $paymentInformation = [
            'paymentMethod' => [
                'method' => 'stripecreditcards',
                'additional_information' => [
                    'number' => '4169160464654525',
                    'exp_month' => '11',
                    'exp_year' => '24',
                    'cvv' => '009',
                ]
    ...
    */    
        ...
        if (isset($orderRes) && (int)str_replace('"', "", $orderRes) > 0) {
            ...
            if (isset(json_decode($cart)->reserved_order_id)) {
                $querymg = DB::connection('mysql_magento')->select('SELECT * FROM `mg_sales_order` WHERE quote_id = ' . $order_model->magento_quote_id);
                        Log::info('magentoquoteid de sales_order');
                        Log::info($querymg);
                $entity = 0;
                if (count($querymg)) {
    ...
    public static function commentOrder($entity,$msg)
    {
        $api = MagentoApi::getInstance();
