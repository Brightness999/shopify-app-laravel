...
angular
    .module("celebrosUI")
    .controller("CelController", ["$scope", "$location", "$timeout", "$window", "celAPI", "celConfig", "celSort", function ($scope, $location, $timeout, $window, celAPI, celConfig, celSort) {
...
angular
    ...
    .directive("celProduct", ['celConfig', function (celConfig) {
        return {
            restrict: "A",
            template: "<div class=\"cuit_item_container\" ng-class=\"{ 'flex-box-column' : display == 'grid', 'flex-box-row' : display == 'list' }\"><!--	<div class=\"quickview\">              <a title=\"Quick View\" class=\"btn btn-xs  btn-quickview\"  data-target=\"#product-details-modal\"  data-toggle=\"modal\"  ng-click=\"store.selected = product\"> Quick View </a>     </div>-->   <a cel-prod-anlx class=\"product-details\" href=\"{{ product.ProductPageUrl }}\" target=\"_self\">        <div class=\"product-image\">       <!--	 <product-image product=\"product\"></product-image>-->            <img ng-src=\"{{product.ImageUrl}}\" on-error=\"{{ settings.ProductResults.NoImageUrl }}\" max-width=\"240px\" max-height=\"300px\" />        </div>               <div class=\"product-info\">            <div class=\"product-title\" ng-bind-html=\"product.Name | trust_html\"></div>            <div class=\"product-sku\">SKU #{{ product.Sku }}</div>             <div ng-if=\"product.is_in_stock==0\" class=\"product-stock\">{{ product.is_in_stock.replace('0','OUT OF STOCK') }}</div>                        <div ng-show=\"display == 'list'\" class=\"product-description hidden-sm\" ng-bind-html=\"product.Description | trust_html\">            </div>        </div>                 </a>    <div class=\"product-shop\" ng-if=\"product.Price !== '0'\">	    	 <div ng-if=\"settings.ProductResults.ShowRatings === true\" class=\"ratings-container\">	            <div class=\"ratings\">	                <span class=\"ratings-stars ratings-stars-off\">â˜…â˜…â˜…â˜…â˜…</span>	                <span class=\"ratings-stars ratings-stars-on\" ng-style=\"{ width: product.RatingStars + '%' }\">â˜…â˜…â˜…â˜…â˜…</span>	            </div>	        </div>    		 <div ng-show=\"product.Original_Price !== undefined && product.Original_Price !== product.Price && product.Original_Price !== '0.00'\" class=\"old-product-price\">{{ product.Original_Price | currency : \"$\" : 2  }}</div>	        <div ng-hide=\"settings.Magento.RealTimePrice === true && !priceLoaded\" class=\"product-price\">{{ product.Price | currency : \"$\" : 2 }}</div>	                 		</div> <!--End addtocard vtex -->		     <div class=\"add-to-cart\">		            <a ng-if=\"product.AddToCart === undefined\" cel-prod-anlx href=\"{{ product.ProductPageUrl }}\" target=\"_self\">{{ settings.Language.ViewDetails }}</a>		            <form ng-if=\"product.AddToCart !== undefined && formKey !== undefined\" data-role=\"tocart-form\" action=\"{{ product.AddToCart }}\" method=\"post\">		                <input type=\"hidden\" name=\"product\" value=\"{{ product.mag_id }}\" />		                <input type=\"hidden\" name=\"uenc\" value=\"{{ uenc }}\" />		                <input type=\"hidden\" name=\"form_key\" value=\"{{ formKey }}\" />		                <div class=\"control\"><input type=\"number\" name=\"qty\" value=\"1\" title=\"Qty\" class=\"input-text qty\" data-validate=\"{'required-number':true,'validate-item-quantity':{'minAllowed':1}}\"></div>		                <button cel-prod-anlx type=\"submit\" title=\"{{ settings.Language.AddToCart }}\" class=\"btn-default\">{{ settings.Language.AddToCart }}</button>		            </form>		      </div>     </div>       <!--ng repeat  <div class=\"modal fade\" id=\"product-details-modal\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\" aria-hidden=\"true\">    <div class=\"modal-dialog\">      <div class=\"modal-content\">        <div class=\"modal-body\">          {{product.name}}        </div>        <div class=\"modal-footer\">          <button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Close</button>        </div>      </div>    </div>  </div>-->   </div>",
            link: function (scope, elem, attr) {
                ...
                if (celConfig.Settings.ProductResults.AddToCartType !== "Default") {
                    ...
                    if (scope.product.AddToCart !== null) {
                        scope.product.AddToCart = scope.product.AddToCart.replace("***window.url***", btoa(location.href));
                        scope.product.AddToCart = scope.product.AddToCart.replace("***product.mag_id***", scope.product.mag_id);
                        scope.product.AddToCart = scope.product.AddToCart.replace("***product.sku***", scope.product.Sku);
                    }
                }
          /* product view
               $scope.store = {
